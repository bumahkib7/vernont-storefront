...
export function VerticalCollectionsClient({ verticalId }: { verticalId: string }) {
  const config = getVertical(verticalId);
  if (!config) return null;

  return <VerticalCollectionsInner config={config} />;
}

function VerticalCollectionsInner({ config }: { config: VerticalConfig }) {
  const { data: collectionsData, isLoading, error } = useCollections({ productType: config.productType });
  ...
  return (
    <PageLayout>
      ...
      {featured && !isLoading && (
        <Link
          href={`/${config.id}/collections/${featured.handle}`}
        ...
        <div className="prose max-w-none">
          <div className="mb-8">
            <p className="text-[15px] leading-relaxed text-neutral-700 mb-4">
              Explore Vernont&apos;s expertly curated {config.label.toLowerCase()} collections, each thoughtfully designed around specific styles, occasions, or trends. Our collections make it easy to discover {config.label.toLowerCase()} that align with your aesthetic preferences, lifestyle needs, or the season&apos;s latest fashion movements. From timeless classics that never go out of style to avant-garde designs pushing the boundaries of contemporary {config.label.toLowerCase()}, each collection tells a unique story through carefully selected pieces from the world&apos;s finest brands.
            </p>
            <p className="text-[15px] leading-relaxed text-neutral-700">
              Whether you&apos;re building a wardrobe of versatile everyday {config.label.toLowerCase()}, seeking statement pieces for special occasions, or looking for performance {config.label.toLowerCase()} for outdoor adventures, our collections provide curated starting points for your perfect discovery journey.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            <div className="bg-white border border-neutral-200 p-6 rounded-lg shadow-sm">
              ...
              <p className="text-[14px] leading-relaxed text-neutral-700 mb-3">
                Our seasonal collections capture the essence of each time of year, featuring {config.label.toLowerCase()} perfectly suited to spring blooms, summer adventures, autumn elegance, and winter sports. Updated quarterly with fresh arrivals and trending styles.
              </p>
            ...
            <div className="bg-white border border-neutral-200 p-6 rounded-lg shadow-sm">
              ...
              <p className="text-[14px] leading-relaxed text-neutral-700 mb-3">
                Curated around how you live and what you love. From beach vacations and urban commutes to mountain adventures and office-ready professional {config.label.toLowerCase()}, these collections match products to your activities and environment.
              </p>
              <p className="text-[13px] text-neutral-600">
            ...
            <div className="bg-white border border-neutral-200 p-6 rounded-lg shadow-sm">
              ...
              <p className="text-[14px] leading-relaxed text-neutral-700 mb-3">
                Exclusive limited-edition collections from designer partnerships and special brand capsule releases. These unique pieces blend fashion house heritage with innovative design for truly distinctive {config.label.toLowerCase()}.
              </p>
              <p className="text-[13px] text-neutral-600">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                ...
                <p className="text-[14px] text-neutral-700 mb-4">
                  Each collection is assembled by our team of specialists and fashion consultants who analyze trends, customer preferences, and seasonal needs. We hand-select every piece to ensure cohesive aesthetics and complementary styles within each collection.
                </p>
                <h3 className="text-[16px] font-medium text-neutral-900 mb-2">Multi-Brand Selections</h3>
                <p className="text-[14px] text-neutral-700">
                  Unlike single-brand showcases, our collections span multiple designers and price points, allowing you to compare styles, discover new brands, and find the perfect {config.label.toLowerCase()} regardless of your budget or brand loyalty.
                </p>
              </div>
              <div>
                ...
                <p className="text-[14px] text-neutral-700">
                  Shopping by collection simplifies your search by pre-filtering thousands of products down to curated selections. Save time, discover unexpected styles, and find {config.label.toLowerCase()} that match your intended use or aesthetic preference.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-neutral-50 p-6 rounded-lg mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[14px]">
              <div>
                ...
                <p className="text-neutral-600 text-[13px]">Customer favorites and top-rated products</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-neutral-600 text-[13px]">Premium products at discounted prices</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-neutral-600 text-[13px]">Oversized, colorful, and dramatic pieces</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-neutral-600 text-[13px]">Performance products for athletes</p>
              </div>
              <div>
          ...
          <div className="border-l-4 border-neutral-300 pl-6">
            ...
            <ul className="space-y-2 text-[14px] text-neutral-700">
              <li className="flex items-start gap-2">
                <span className="text-neutral-500 mt-1">&bull;</span>
              ...
              <li className="flex items-start gap-2">
                <span className="text-neutral-500 mt-1">&bull;</span>
                <span><strong>Check Back Regularly:</strong> Collections update frequently with new additions, so your perfect product might arrive next week</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-neutral-500 mt-1">&bull;</span>
                <span><strong>Save Favorites:</strong> Use your wishlist to bookmark products from different collections for easy comparison later</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <section className="py-16 lg:py-24">
        <div className="max-w-[1500px] mx-auto px-6 lg:px-20">
          ...
          {featured && !isLoading && (
            <div className="mb-10">
              <h2 className="text-2xl lg:text-3xl font-medium tracking-tight">{config.label} Collections</h2>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20">
              ...
              <Link href={config.catalogPath} className="inline-flex items-center gap-2 px-8 py-3 bg-[var(--foreground)] text-[var(--background)] text-sm font-medium">
                Shop All {config.label} <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
              {(featured ? rest : collections).map((collection, index) => (
                <Link
                  key={collection.id}
                  href={`/${config.id}/collections/${collection.handle}`}
      ...
      <section className="py-16 lg:py-20 bg-[var(--surface)]">
        <div className="max-w-[1500px] mx-auto px-6 lg:px-20 text-center">
          ...
          <p className="text-[var(--muted-foreground)] mb-8 max-w-md mx-auto">
            Browse all our {config.label.toLowerCase()} or use our filters to find your perfect match.
          </p>
          <Link
