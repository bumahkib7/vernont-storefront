...
// SEO content (guarantees, styles, FAQs) is now driven by the vertical config's seoContent field

function CatalogPageContent({ config }: { config: VerticalConfig }) {
  ...
  const apiParams = useMemo(() => {
    const params: Parameters<typeof useProducts>[0] = { limit: PAGE_SIZE, page, productType: config.productType };
  ...
  const pageTitle = useMemo(() => {
    const saleParam = searchParams.get('sale');
    if (saleParam === 'true') {
      return `Sale ${config.label}`;
    }
    if (selectedBrands.length === 1) {
      const brand = brands.find(b => b.id === selectedBrands[0]);
      if (brand) return `${brand.name} ${config.label}`;
    }
    if (selectedBrands.length > 1) {
      return `Designer ${config.label}`;
    }
          ...
          <div className="px-4 lg:px-6 py-3">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <SortDropdown value={sortBy} onChange={(v) => { setSortBy(v); setPage(0); }} options={SORT_OPTIONS} />
              </div>
            </div>
            ...
            <div className="max-w-[1200px] mx-auto px-4 lg:px-8 py-20">
              ...
              <div className="max-w-[720px] mx-auto text-center mb-20">
                <p className="text-[10px] tracking-[0.3em] uppercase text-[#999] mb-4">
                  {seoContent.editorialIntro.label}
                </p>
                <h2
                ...
                >
                  {seoContent.editorialIntro.headline}
                </h2>
                <p className="text-[15px] leading-relaxed text-[#555]">
                  {seoContent.editorialIntro.body}
                </p>
              </div>
              ...
              <div className="mb-20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {seoContent.guarantees.map(({ title, body }) => (
                    <div
                      key={title}
                    ...
                    >
                      <div className="w-11 h-11 rounded-full bg-[#F5F5F5] flex items-center justify-center mb-4">
                        <Medal className="w-5 h-5 text-[#1A1A1A]" weight="regular" />
                      </div>
              ...
              {/* Featured editorial block — optional per vertical */}
              {seoContent.featuredEditorial && (
                <div className="mb-20 bg-white border border-[#E5E5E5] rounded-sm p-8 md:p-12">
                  <div className="grid md:grid-cols-[1fr_2fr] gap-8 md:gap-16 items-start">
                    <div>
                      <p className="text-[10px] tracking-[0.3em] uppercase text-[#999] mb-3">{seoContent.featuredEditorial.sectionLabel}</p>
                      <h3
                        className="text-2xl md:text-3xl text-[#1A1A1A] leading-tight"
                        style={{ fontFamily: "'Crimson Pro', 'Georgia', serif", fontWeight: 400 }}
                      >
                        {seoContent.featuredEditorial.heading}
                      </h3>
                    </div>
                    <div className="space-y-4 text-[14px] leading-relaxed text-[#555]">
                      {seoContent.featuredEditorial.paragraphs.map((p, i) => (
                        <p key={i}>{p}</p>
                      ))}
                    </div>
                  </div>
              ...
              <div className="mb-20">
                <div className="text-center mb-10">
                  <p className="text-[10px] tracking-[0.3em] uppercase text-[#999] mb-3">{seoContent.productStyles.sectionLabel}</p>
                  <h3
                    className="text-2xl md:text-3xl text-[#1A1A1A]"
                    style={{ fontFamily: "'Crimson Pro', 'Georgia', serif", fontWeight: 400 }}
                  >
                    {seoContent.productStyles.heading}
                  </h3>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {seoContent.productStyles.items.map(({ title, body }) => (
                    <div
                      key={title}
...
export function CatalogPageClient({ verticalId }: { verticalId: string }) {
    ...
    return (
      <div className="min-h-screen bg-[var(--background)]">
        ...
        <main className="max-w-7xl mx-auto px-4 py-16 text-center">
          <p className="text-lg text-[var(--muted-foreground)]">Unknown product category.</p>
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <Suspense fallback={
      ...
      <CatalogPageContent config={config} />
    </Suspense>
  );
}
