...
import { CaretDown, Sparkle, Heart } from "@phosphor-icons/react";
...
export default function ForHerPage() {
  ...
  // Transform, filter, and sort products for women
  const displayProducts = useMemo(() => {
    if (!productsData?.items) return [];
    const allProducts = transformProducts(productsData.items);
    let womenProducts = filterByGender(allProducts, 'women');

    switch (sortBy) {
      case "price-low":
        return [...womenProducts].sort((a, b) => a.price - b.price);
      case "price-high":
        return [...womenProducts].sort((a, b) => b.price - a.price);
      case "newest":
        return [...womenProducts].sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0));
      default:
